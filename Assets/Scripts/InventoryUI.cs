...
public class InventoryUI : MonoBehaviour
{
    public Sprite defaultBackgroundSprite; // Ảnh nền mặc định
public Sprite selectedBackgroundSprite;
    ...
    public void SelectItem(Item item, Transform itemSlotTransform)
{
    if (selectedItemSlot != null)
    {
        // Reset ảnh nền của item trước đó về mặc định
        Image prevBackground = selectedItemSlot.Find("Background")?.GetComponent<Image>();
        if (prevBackground != null)
        {
            prevBackground.sprite = defaultBackgroundSprite; // Thay bằng sprite mặc định của bạn
        }
    }
    ...
    {
        selectedItem = item;
        selectedItemSlot = itemSlotTransform;

        // ✅ Đổi Background khi chọn
        Image background = selectedItemSlot.Find("Background")?.GetComponent<Image>();
        if (background != null)
        {
            background.sprite = selectedBackgroundSprite; // Thay bằng sprite khi chọn
        }

        Debug.Log($"✅ Đã chọn item: {item.itemType}, Số lượng: {item.amount}");
    }
    else
    {
        Debug.LogWarning("⚠ Không thể chọn item vì nó NULL!");
...
if (button != null)
{
    ...
    button.onClick.AddListener(() => SelectItem(item, itemSlotTransform)); // Truyền item slot
}
            else
            {
                Debug.LogWarning("⚠ Item slot không có Button component!");
